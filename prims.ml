...
module Prims : PRIMS = struct
  ...
  let misc_ops =
    ...
    let our_funcs=
  let car= make_unary "car" "mov rax, qword[rsi+ TYPE_SIZE]" in
  let cdr= make_unary "cdr" "mov rax, qword [rsi + TYPE_SIZE + WORD_SIZE]" in
  let cons = make_binary "cons" "MAKE_PAIR(rax,rsi,rdi)" in
  let set_car= make_binary "set_car" 
  "mov  qword [rsi+TYPE_SIZE], rdi\nmov rax, SOB_VOID_ADDRESS\n" in
  let set_cdr = make_binary "set_cdr" 
  "mov qword [rsi + TYPE_SIZE + WORD_SIZE], rdi\n mov rax, SOB_VOID_ADDRESS\n" in

 
 (* 1. Check if last argument is list
  2. push that list to stack in reversed oreder (1, 2, 3) -> push 3, push 2, push 1
  3. push other args
  4. push new number of total args (number of old + (list length) -2)
  5. push old env
  6. push ret address
  8. shift the frame up with offset (4 + number of old args)
          *let popTImes= (old fram size) - (new frame size)
          while(popTimes>0)
              pop
              popTimes--;
  8. rbp=r15 the old rbp
  9. call the new function (the closure we got as argument before) 
  *)
  ...
  "apply:
        push rbp
        mov rbp, rsp
        mov r15, qword [rbp]    ;save the old rbp for later on.

        ;Check if last argument is a list
        mov r8, PARAM_COUNT  ;r8 = n
        dec r8
        mov r9, PVAR(r8)    ;r9=the list.
        cmp byte [r9], T_NIL  ;check if the list is null
        je .list_is_empty


        mov r10, 0          ;list length counter.

        ;PUSH_LIST TESTED AND WORK!
        .push_list:
        cmp byte [r9], T_NIL  ;check if the list is null
        je .reverse_list_on_stack
        CAR r8, r9             ;r8 = car(r9)
        push r8
        CDR r9, r9
        inc r10
        ...
        .reverse_list_on_stack:     ;r10= list length
        mov r11,r10
        dec r11
        mov r8, 0

        ; reverse_list_loop TESTED AND WORKS!
        .reverse_list_loop:
        cmp r11,r8
        jle .finished_reverse
        mov r12, qword [rsp + 8*r11]  ;r12 is tmp
        mov r13, qword [rsp + 8*r8]   ;r13 is tmp
        mov [rsp+ 8*r11], r13         ;swap
        mov [rsp+ 8*r8], r12          ;swap
        inc r8
        dec r11
        ...
        ;push_args_loop TESTED AND WORKS!
        .push_args_loop:
        cmp r8,0
        je .set_new_stack_frame
        mov r9, PVAR(r8)
        push r9
        dec r8
        jmp .push_args_loop
        ...
        ;set_new_stack TESTED AND WORKS!
        .set_new_stack_frame:
        mov r8, PARAM_COUNT       
        add r8, r10             ;add number of args in list
        sub r8, 2               ;r8= new params_count (we subbed f and list)
        push r8
        mov r8, qword [rbp + 4*8]
        CLOSURE_CODE rbx, r8     ;rbx is the closure code for later use.
        CLOSURE_ENV r9, r8
        push r9                 ;push the closure env
       ...
       .prepare_for_shift:
        add r10, PARAM_COUNT 
        mov r14, PARAM_COUNT
        add r14,4                ;r14=old frame size. (offset)            
        sub r10, 2               ;remove lst and proc from args counter
        add r10, 3               ;r8=new stack frame size
        mov r9, PARAM_COUNT
        add r9,3                 ;j = n+3 when n is PARAM_COUNT
        mov r8, r10
        
        ;shift_frame TESTED AND WORKS!
        .shift_frame:
        cmp r8, 0               ;new stack frame size + 1
        je .end_shift_frame
        dec r8
        mov r11, qword [rsp+8*r8]
        mov [rbp+8*r9],r11
        dec r9
        jmp .shift_frame
        ...
        pop rbp
        mov rbp, r15
        jmp rbx               ;rbx is the closure code!
        ...
        CLOSURE_CODE rbx, r8     ;rbx is the closure code for later use.
        CLOSURE_ENV r9, r8
        mov qword [rbp+2*8], r9


        ;shift all up by 1
        mov r8, PARAM_COUNT
        dec r8
        add r8,4              ;offset to the last arg (without the list)

        .list_empty_loop:
        cmp r8,0
        je .list_empty_looop_end
        dec r8
        mov r10,r8
        inc r10
        mov r9, qword [rbp+8*r8]
        mov qword [rbp+8*r10], r9
        jmp .list_empty_loop

        .list_empty_looop_end:
        add rsp,8
        add rbp,8
        ;change param count to p.c -1.
        mov r8, PARAM_COUNT
        dec r8
        dec r8
        mov qword[rbp+3*8], r8        ;set new param count.



        ;shift the rest 1 up
        mov r8, PARAM_COUNT
        mov qword[rbp+8*4], r8
        ...
        jmp .finish_apply      ;make sure rbx has the closure code! 
  ...
  let procs = String.concat "\n\n" [type_queries ; numeric_ops; misc_ops;our_funcs];;
end;;
