...
(define fold-left (lambda (ff accc lstt)
    (letrec (( fold (lambda (f acc lst)
        			(if (eq? '() lst)
            			acc
  					(fold f (f acc (car lst)) (cdr lst)))))) (fold ff accc lstt))))



(define fold-right (lambda (ff accc lstt)
    (letrec ((fold (lambda (f acc lst)
      (if (eq? '() lst)
          acc
          (f (car lst) (fold f acc (cdr lst))))))) (fold ff accc lstt))))

(define get_last (lambda (lst)
   (letrec ((f (lambda (lstt) 
   		(if (eq? (cdr lstt) '()) (car lstt)
        (f (cdr lstt)))     ))) (f lst) )))

(define cut_last (lambda (lst)
	  (letrec ((f (lambda (lstt)
	   (if (eq? (cdr lstt) '()) '()
       (cons (car lstt) (f (cdr lstt))))))) (f lst))))

(define (cons* . lst)
   (if (eq? (cdr lst) '()) (car lst)
   (fold-right cons (get_last lst) (cut_last lst))  ))
